...
// Replace target 1
const newTargetRegex = /\\{\\s*newTracks\\.length === 0 && \\(\\s*<div className="text-sm text-slate-500 font-mono py-12 text-center border border-dashed border-white\\/5 rounded-2xl">\\s*No new releases available at the moment\\.\\s*<\\/div>\\s*\\)\\}/;

content = content.replace(newTargetRegex, newInjection + '\\n                      {newTracks.length === 0 && (\\n                        <div className="text-sm text-slate-500 font-mono py-12 text-center border border-dashed border-white/5 rounded-2xl">\\n                          No new releases available at the moment.\\n                        </div>\\n                      )}');

// Replace target 2
const radioTargetRegex = /\\{\\/\\* Subscribe to Play Episodes \\*\\/\\}/;

content = content.replace(radioTargetRegex, radioInjection + '\\n                      {/* Subscribe to Play Episodes */}');
