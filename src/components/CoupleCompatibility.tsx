...
import { Heart, Copy, Check, X, Music, Users, Sparkles, Star, Flame, Headphones, Trophy } from 'lucide-react';
...
function calculateCompatibility(a: CoupleProfile, b: CoupleProfile): CompatibilityResult {
  ...
  const songScore = a.likedTracks.length + b.likedTracks.length > 0
    ? (sharedSongs.length * 2) / (a.likedTracks.length + b.likedTracks.length) * 100
    : 0;

  const artistScore = a.likedArtists.length + b.likedArtists.length > 0
    ? (sharedArtists.length * 2) / (a.likedArtists.length + b.likedArtists.length) * 100
    : 0;
...
export const CoupleCompatibility: React.FC<CoupleCompatibilityProps> = ({ currentUser, allTracks, onClose }) => {
  const [step, setStep] = useState<'share' | 'waiting' | 'result'>('share');
  const [linkCopied, setLinkCopied] = useState(false);
  const [partnerData, setPartnerData] = useState<CoupleProfile | null>(null);
  const [result, setResult] = useState<CompatibilityResult | null>(null);

  // Encode user profile into a compact URL-safe payload
  const payload = btoa(JSON.stringify({
    id: currentUser.id,
    displayName: currentUser.displayName,
    avatarUrl: currentUser.avatarUrl,
    likedTracks: currentUser.likedTracks.slice(0, 50), // cap for URL size
    ...
    if (coupleParam) {
      try {
        ...
        if (decoded.id !== currentUser.id) {
          ...
          const compatibility = calculateCompatibility(currentUser, decoded);
  ...
  const handleSimulate = () => {
    // Demo: simulate a partner for preview purposes
    const fakePartner: CoupleProfile = {
      id: 'demo-partner',
      displayName: 'Mystery Partner 🎵',
      avatarUrl: 'https://picsum.photos/seed/partner/400/400',
      likedTracks: currentUser.likedTracks.slice(0, Math.floor(currentUser.likedTracks.length * 0.6)),
      likedArtists: currentUser.likedArtists.slice(0, Math.floor(currentUser.likedArtists.length * 0.5)),
      stats: { favArtist: currentUser.stats.favArtist, topGenre: currentUser.stats.topGenre, tracksPlayed: 80, minutesListened: 200 },
    };
    setPartnerData(fakePartner);
    const comp = calculateCompatibility(currentUser, fakePartner);
    setResult(comp);
    setStep('result');
  };
      ...
      >
        ...
        <div className="p-8">
          <AnimatePresence mode="wait">
            ...
            {step === 'share' && (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl w-full"
                  ...
                  <img
                    src={currentUser.avatarUrl || 'https://picsum.photos/seed/default/100'}
                ...
                <p className="text-xs text-slate-400">Send this link to someone special. When they open it, you'll both see your music compatibility score!</p>
                ...
                <div className="flex w-full gap-2">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl text-xs font-bold text-slate-400 transition-all hover:text-white"
                    style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}
                  >
                    Preview Demo Score
                  </button>
            ...
            {/* STEP 3: RESULT */}
            {step === 'result' && result && partnerData && (
              <motion.div
                key="result"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <motion.img
                    initial={{ x: 20, opacity: 0 }}
                    animate={{ x: 0, opacity: 1 }}
                    src={partnerData.avatarUrl || 'https://picsum.photos/seed/partner/100'}
                    className="w-16 h-16 rounded-full object-cover ring-4"
                    ...
                    <div className="flex flex-col gap-1">
                      {sharedSongNames.map(t => (
                        <div key={t.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs"
                          ...
                          <span className="text-slate-500 truncate">{t.artist}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <button
                  onClick={() => { setStep('share'); setResult(null); setPartnerData(null); }}
                  className="w-full py-2.5 rounded-xl text-xs font-bold text-slate-400 transition-all hover:text-white mt-2"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}
                >
