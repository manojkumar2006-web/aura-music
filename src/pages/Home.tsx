...
export const Home: React.FC = () => {
  ...
  // Throttled artist image prefetch on startup — fetches one artist every 150ms
  // to avoid flooding the Deezer API with 100+ simultaneous requests during initial render
  useEffect(() => {
    const { artistImages, fetchArtistImage, tracks } = useMusicStore.getState();
    const allPersons = new Set();
    tracks.forEach(t => {
      if (t.artist) t.artist.split(', ').forEach(a => a.trim() && allPersons.add(a.trim()));
      if (t.hero) allPersons.add(t.hero);
      if (t.musicDirector) allPersons.add(t.musicDirector);
    });
    const toFetch = [...allPersons].filter(name => {
      const inInstant = Object.keys(INSTANT_ARTIST_PHOTOS).some(k => k.toLowerCase() === name.toLowerCase());
      return !inInstant && !artistImages[name];
    });
    let i = 0;
    const interval = setInterval(() => {
      if (i >= toFetch.length) { clearInterval(interval); return; }
      fetchArtistImage(toFetch[i]);
      i++;
    }, 150);
...
const handlePlayNext = (e: React.MouseEvent, track: Track) => {
 ...
 className="w-full bg-[#121212] border border-white/10 rounded-[2rem] p-8 shadow-[0_20px_70px_rgba(0,0,0,0.8)] relative overflow-hidden group"
 >
 {/* Edge Glow */}
 ...
 {authError && (
 <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-3 mb-6 rounded-xl border border-red-500/20 text-[#ff6b6b] bg-red-500/10 text-xs font-semibold flex items-center gap-2 ">
 ...
 {authSuccess && (
 <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-3 mb-6 rounded-xl border text-[color:var(--color-teal)] text-xs font-semibold flex items-center gap-2 " style={{ backgroundColor: 'rgba(147,177,166,0.1)', borderColor: 'rgba(147,177,166,0.2)' }}>
 ...
 required
 className="peer w-full bg-[#181818] border border-white/10 rounded-xl pt-5 pb-2 pl-11 pr-4 text-sm text-white placeholder-transparent focus:outline-none focus:border-[color:var(--color-teal)] focus:bg-[#242424] focus:shadow-[0_0_15px_var(--color-teal)] transition-all"
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="absolute inset-0 bg-[#121212] cursor-pointer"
 ...
 {p.coverUrl ? (
 <img loading="lazy" src={p.coverUrl} className="w-full h-full object-cover" alt="" />
 ) : (
 ...
 <span className="text-[9px] uppercase font-mono tracking-widest text-slate-300 font-bold bg-[#242424] px-2 py-0.5 rounded border border-white/20">
 Free Player
 </span>
 )}
 </div>
 ...
 <button
 onClick={() => setTier('Free')}
 className="w-full py-2 bg-[#181818] hover:bg-deepblue/10 border border-white/10 hover:border-deepblue/30 text-ink-secondary hover:text-ink-primary text-xs rounded-xl transition-all cursor-pointer font-mono"
 >
 ...
 <button
    ...
    className="md:hidden glass-panel rounded-2xl px-4 py-2 border border-silver/8 bg-graphite/45 flex items-center gap-2 hover:border-teal/30 hover:bg-teal/5 hover:text-teal transition-all cursor-pointer h-12 flex-shrink-0 text-slate-300 shadow-lg"
    title="Download App"
  >
 ...
 : 'bg-[#181818] border-white/10 text-ink-secondary'
 }`}>
 {userTier}
 </span>
 </div>
 ...
 className="py-1.5 px-4 bg-[#181818] hover:bg-[#242424] border border-white/5 hover:border-white/10 text-ink-primary font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
 >
 <Edit2 className="w-3.5 h-3.5 text-ink-secondary" /> Edit Profile
 </button>
 <button
 onClick={() => setShowSleepTimerModal(true)}
 className={`py-1.5 px-4 ${sleepTimerRemaining ? 'bg-teal/10 hover:bg-teal/20 border border-teal/20 hover:border-teal/30 text-teal' : 'bg-[#181818] hover:bg-[#242424] border border-white/5 hover:border-white/10 text-ink-primary'} font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer`}
 >
 ...
 required
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-teal/40"
 />
 </div>
 <div className="flex flex-col gap-1.5">
 ...
 className="bg-[#121212] border border-white/10 rounded-xl py-1.5 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-teal/40 file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-[10px] file:font-semibold file:bg-teal/20 file:text-teal hover:file:bg-teal/30 cursor-pointer"
 />
 </div>
 </div>
 ...
 title={avatar.name}
 >
 <img loading="lazy" src={avatar.url} className="w-full h-full object-cover" alt="" />
 </button>
 ))}
 </div>
 </div>
 ...
 rows={3}
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-teal/40 font-sans"
 />
 </div>
 ...
 className="py-2 px-4 bg-[#181818] hover:bg-[#242424] rounded-xl text-ink-secondary hover:text-white text-xs font-semibold cursor-pointer"
 >
 Cancel
 </button>
 <button
 ...
 currentUser.privacy.isPublicProfile ? 'bg-teal' : 'bg-[#242424]'
 }`}
 >
 <span
 ...
 className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
 isEqualizerEnabled ? 'bg-teal' : 'bg-[#242424]'
 }`}
 >
 <span
 className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
 ...
 <div className="bg-[#121212] border border-white/5 rounded-2xl p-4 flex justify-between items-end h-40 gap-2">
 {eqBands.map((val, idx) => {
 ...
 className="absolute w-20 h-1.5 bg-[#242424] rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-teal [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(24, 61, 61,0.6)] outline-none origin-center -rotate-90 hover:[&::-webkit-slider-thumb]:scale-110 transition-all"
 />
 </div>
 ...
 className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
 currentUser.privacy.showListeningActivity ? 'bg-teal' : 'bg-[#242424]'
 }`}
 >
 <span
 className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
 ...
 className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
 currentUser.privacy.playlistsPrivateByDefault ? 'bg-teal' : 'bg-[#242424]'
 }`}
 >
 <span
 className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
...
>
 ...
 className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer group transition-colors ${activeTrackMenu === track.id ? 'z-30 relative' : ''} ${isPlaying ? 'bg-[#242424]' : 'hover:bg-[#181818]'}`}
 >
 <div className="w-8 flex justify-center">
 {isPlaying ? (
 <div className="flex gap-1 items-end h-4">
 <div className="w-1 bg-teal rounded-full h-2 animate-[bounce_0.8s_infinite]" />
 <div className="w-1 bg-teal rounded-full h-3 animate-[bounce_0.8s_infinite_0.15s]" />
 <div className="w-1 bg-teal rounded-full h-2.5 animate-[bounce_0.8s_infinite_0.3s]" />
 </div>
 ) : (
 ...
 <img loading="lazy" src={coverImage} alt={selectedPlaylist} className="w-full h-full object-cover" />
 </div>
 ...
 className={`group grid grid-cols-[16px_1fr_40px] md:grid-cols-[24px_1fr_1fr_40px] gap-4 items-center px-4 py-3 rounded-xl cursor-pointer transition-colors ${isPlaying ? 'bg-[#242424]' : 'hover:bg-[#181818]'}`}
 >
 ...
 <img loading="lazy" src={track.coverUrl} className="w-10 h-10 rounded shadow-md hidden sm:block" alt={track.title} />
 ...
 className="opacity-100 md:opacity-0 md:group-hover:opacity-100 p-2 hover:bg-[#242424] rounded-full transition-all text-slate-400 hover:text-white cursor-pointer"
 >
 <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>
 </button>
 ...
 <div className="absolute top-2.5 right-2.5 bg-teal/90 rounded-full p-2.5 opacity-0 group-hover:opacity-100 transition-all translate-y-1 group-hover:translate-y-0 shadow-[0_0_20px_rgba(20,184,166,0.6)]">
 ...
 <span className="px-3 py-1 bg-[#242424] rounded-full text-xs font-bold text-white uppercase tracking-wider border border-white/5 flex items-center gap-1.5">
 {currentWeather === 'Rainy' || currentWeather === 'Stormy' ? '🌧️' : currentWeather === 'Snowy' ? '❄️' : currentWeather === 'Sunny' ? '☀️' : '☁️'} {currentWeather} Day
 </span>
 <span className="px-3 py-1 bg-teal/20 text-teal rounded-full text-xs font-bold uppercase tracking-wider border border-teal/20">
 {userRegion}
 </span>
 </div>
 ...
 {(mix as any).coverUrl ? (
 <img loading="lazy" src={(mix as any).coverUrl} className="w-full h-full object-cover" alt={mix.title} />
 ) : (
 ...
 <div key={track.id} onClick={() => handleSelectTrack(track)} className="glass-panel p-2 rounded-xl flex items-center gap-3 cursor-pointer hover:bg-[#181818] transition-all border border-transparent hover:border-white/10 group">
 <img loading="lazy" src={track.coverUrl} className="w-12 h-12 rounded-lg object-cover shadow-md" alt={track.title} />
 <div className="flex flex-col flex-1 min-w-0">
 ...
 required
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/40"
 />
 </div>
 <div className="flex flex-col gap-1">
 ...
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 focus:outline-none focus:border-emerald-500/40"
 >
 ...
 className="bg-[#121212] border border-white/10 rounded-xl py-1.5 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/40 file:mr-3 file:py-1 file:px-2 file:rounded-md file:border-0 file:text-[9px] file:font-semibold file:bg-emerald-500/20 file:text-emerald-400 hover:file:bg-emerald-500/30 cursor-pointer"
 />
 </div>
 </div>
 ...
 required
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/40 font-mono"
 />
 </div>
 <div className="flex flex-col gap-1">
 ...
 <div className="flex items-center justify-between bg-[#121212] border border-white/10 rounded-xl p-2.5">
 <div className="flex flex-col">
 ...
 rows={2}
 className="bg-[#121212] border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/40 font-sans"
 />
 </div>
 ...
 {filteredTracks.slice(0, visibleLibraryCount).map((track, idx) => {
 ...
 className={`p-0.5 rounded-full text-slate-400 hover:text-white hover:bg-[#242424] transition-all cursor-pointer ${activeTrackMenu === track.id ? 'bg-[#242424] text-white' : ''}`}
 >
 ...
 exit={{ opacity: 0, scale: 0.95, y: -10 }}
 transition={{ duration: 0.15 }}
 className="absolute right-0 top-full mt-1 w-40 rounded-xl bg-[#0f172a] border border-white/10 shadow-2xl py-1.5 z-50 flex flex-col text-left"
 onClick={(e) => e.stopPropagation()}
 >
 <button
 onClick={(e) => handleAddToQueue(e, track)}
 className="w-full px-3 py-1.5 text-[11px] font-semibold text-slate-200 hover:text-white hover:bg-[#242424] flex items-center gap-2 transition-colors text-left"
 >
 <PlusCircle className="w-3.5 h-3.5 text-teal" />
 Add to Queue
 </button>
 ...
 { name: 'Concerts', bg: 'from-[#e44d26] to-[#f16529]', emoji: '🎪', img: 'https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?q=80&w=400&auto=format&fit=crop' },
 { name: 'Punjabi', bg: 'from-[#8e2de2] to-[#4a00e0]', emoji: '🥁', img: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=400&auto=format&fit=crop' },
 { name: 'Tamil', bg: 'from-[#b721ff] to-[#21d4fd]', emoji: '🎵', img: 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=400&auto=format&fit=crop' },
 { name: 'Classical', bg: 'from-[#c79081] to-[#dfa579]', emoji: '🎻', img: 'https://images.unsplash.com/photo-1459749411175-04bf5292ceea?q=80&w=400&auto=format&fit=crop' },
 ...
 { name: 'R&B/Soul', bg: 'from-[#fc4a1a] to-[#f7b733]', emoji: '💿', img: 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=400&auto=format&fit=crop' },
 ...
 { name: 'Indie', bg: 'from-[#ee9ca7] to-[#ffdde1]', emoji: '🌿', img: 'https://images.unsplash.com/photo-1601643157091-ce5c665179ab?q=80&w=400&auto=format&fit=crop' },
 { name: 'K-Pop', bg: 'from-[#ff6a88] to-[#ff99ac]', emoji: '💖', img: 'https://images.unsplash.com/photo-1619983081563-430f63602796?q=80&w=400&auto=format&fit=crop' },
 { name: 'Devotional', bg: 'from-[#f2994a] to-[#f2c94c]', emoji: '🙏', img: 'https://images.unsplash.com/photo-1498038432885-c6f3f1b912ee?q=80&w=400&auto=format&fit=crop' },
 { name: 'Chill', bg: 'from-[#0f2027] via-[#203a43] to-[#2c5364]', emoji: '🌙', img: 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=400&auto=format&fit=crop' },
 ...
 { name: 'Lo-Fi', bg: 'from-[#2c3e50] to-[#4ca1af]', emoji: '☕', img: 'https://images.unsplash.com/photo-1518199266791-5375a83190b7?q=80&w=400&auto=format&fit=crop' },
 ].map((cat, idx) => (
 <div
 key={cat.name}
 data-scroll-reveal
 ...
 >
 <div className="w-full aspect-square rounded-xl overflow-hidden relative shadow-md bg-[#121212] premium-image-hover">
 <img loading="lazy" src={track.coverUrl} className="w-full h-full object-cover" alt={track.title} />
 <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors" />
 ...
 className="grid grid-cols-[16px_1fr_120px_50px] gap-4 px-4 py-3 items-center rounded-xl hover:bg-[#181818] group cursor-pointer transition-colors"
 >
 ...
 <div className="flex items-center gap-3 min-w-0">
 <img loading="lazy" src={track.coverUrl} className="w-10 h-10 rounded-md object-cover shadow-md" alt="" />
 <div className="flex flex-col min-w-0">
 ...
 <span className="text-[10px] font-bold uppercase tracking-widest text-teal bg-[#121212] px-3 py-1 rounded-full border border-teal/20 shadow-lg">
 ...
 <div className="w-10 h-10 rounded-full bg-[#242424] flex items-center justify-center text-white opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 border border-white/20">
 ...
 isSelected 
 ? 'bg-[#242424] border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.05)]' 
 : 'hover:bg-[#181818] hover:border-white/10'
 }`}
 >
 ...
 className={`p-2 rounded-full transition-all text-slate-400 hover:text-white hover:bg-[#242424] ${activeTrackMenu === track.id ? 'opacity-100 bg-[#242424] text-white' : 'opacity-100 md:opacity-0 md:group-hover:opacity-100'}`}
 >
...
\n {newTracks.length === 0 && (
 ...
 className="bg-[#121212] border border-white/10 rounded-xl py-2.5 px-4 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-teal/50"
 />
 </div>
 <div className="flex-1 flex flex-col gap-1.5">
 ...
 className="bg-[#121212] border border-white/10 rounded-xl py-1.5 px-3 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-teal/50 file:mr-3 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-[9px] file:font-semibold file:bg-teal/10 file:text-teal hover:file:bg-teal/20 cursor-pointer"
 />
 </div>
 </div>
 <button
 ...
 className="p-1.5 text-slate-400 hover:text-white hover:bg-[#242424] rounded-full transition-colors"
 >
 <MoreVertical className="w-4 h-4" />
 </button>
 ...
 className="w-full px-4 py-2.5 text-left text-[11px] font-bold text-ink-primary hover:bg-[#181818] hover:text-teal flex items-center gap-2 transition-colors cursor-pointer"
 >
 ...
 <div className="w-10 h-10 rounded-full bg-[#333333] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 border border-white/20">
 ...
 <img loading="lazy" src={podcast.img} className="w-full h-full object-cover" alt={podcast.title} />
 <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors" />
 ...
 <div className="absolute bottom-2 right-2 bg-[#333333] text-white p-2.5 rounded-full opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 shadow-lg translate-y-2 group-hover:translate-y-0 border border-white/20">
...
\n {/* Subscribe to Play Episodes */}
 ...
 <img loading="lazy" src={episode.cover} className="w-full h-full object-cover" alt="" />
 <div className="absolute top-1.5 right-1.5 bg-[#121212] rounded px-1 py-0.5 flex items-center gap-1 ">
 ...
 <button className="ml-auto opacity-100 md:opacity-0 md:group-hover:opacity-100 p-2 hover:bg-[#242424] rounded-full transition-all text-slate-400 hover:text-white" onClick={(e) => e.stopPropagation()}>
 <MoreVertical className="w-4 h-4" />
 </button>
 </div>
 ))}
 ...
 className="relative bg-[#1a1a1a] border border-white/10 rounded-[32px] p-8 max-w-sm w-full shadow-2xl flex flex-col gap-6 text-center"
 onClick={(e) => e.stopPropagation()}
 >
 ...
 className="py-3 bg-[#181818] hover:bg-[#242424] border border-white/10 rounded-xl text-white font-bold transition-all shadow-lg hover:border-teal/30 hover:bg-teal/5"
 >
 {mins} mins
 </button>
 ))}
 ...
 <button 
 onClick={() => setShowSleepTimerModal(false)}
 className="absolute top-6 right-6 p-2 rounded-full hover:bg-[#242424] text-slate-400 hover:text-white transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 </motion.div>
 ...
 animate={{ opacity: 1, scale: 1, y: 0 }}
 exit={{ opacity: 0, scale: 0.95, y: 20 }}
 className="relative bg-[#1a1a1a] border border-white/10 rounded-[32px] p-8 max-w-lg w-full shadow-2xl flex flex-col gap-6"
 onClick={(e) => e.stopPropagation()}
 >
 <button 
 onClick={() => setShowTipsModal(false)}
 ...
 <div className="w-12 h-12 rounded-xl bg-[#181818] border border-white/10 flex items-center justify-center flex-shrink-0 group-hover:bg-teal/20 group-hover:border-teal/30 transition-colors">
 ...
 className={`fixed bottom-28 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-6 py-3 rounded-full shadow-2xl border ${
 ...
 <nav
 className="fixed bottom-0 left-0 right-0 z-40 flex md:hidden items-center justify-around px-1 py-2 bg-[#0e0e10] border-t border-white/[0.06]"
